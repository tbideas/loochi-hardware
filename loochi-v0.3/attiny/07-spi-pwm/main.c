...
ISR(SIG_USI_OVERFLOW)
{
	 ...
	 */
	if (usi_timeout >= USI_TIMEOUT) 
		usi_counter = 0;
		
	usi_buffer[usi_counter++] = USIDR;

	if (usi_counter == 3) {
		pwm_t = 2 * usi_buffer[0];
		usi_counter = 0;
	}
...
int main(void)
{
	...
	USISR = (1 << USIOIF); // make sure the interrupt flag is cleared before we start.
